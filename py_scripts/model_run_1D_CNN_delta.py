...
import time # Import indispensable pour le chronomètre

# ---------------------------------------------------------
# 1. CONFIGURATION MEDIAPIPE
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 2. SCRIPT D'INFÉRENCE EN TEMPS RÉEL (AVEC VERROUILLAGE 3S)
# ---------------------------------------------------------
def run_realtime_inference(model_path="gesture_model_1dcnn.keras", 
    ...
    # --- VARIABLES DE VERROUILLAGE TEMPOREL ---
    stable_gesture = None
    last_detection_time = 0
    DISPLAY_DURATION = 3.0 # Durée d'affichage en secondes
    ...
    while True:
        ...
        # LOGIQUE D'INFÉRENCE ET D'AFFICHAGE
        ...
        if stable_gesture and time_since_detection < DISPLAY_DURATION:
            ...
            # Optionnel : Petite barre de progression visuelle pour les 3s
            bar_width = int((remaining / DISPLAY_DURATION) * 200)
            cv2.rectangle(frame, (15, 50), (15 + bar_width, 55), (0, 255, 0), -1)
            
        # B. MODE ANALYSE (Le temps est écoulé ou rien n'a encore été détecté)
        else:
                ...
                if hand_detected and total_movement < 0.0005:
                    ...
                    if len(prediction_history) == prediction_history.maxlen:
                        ...
                        if pred_counts[best_pred] >= 5 and best_pred != "Inconnu":
                            ...
                            last_detection_time = time.time() # On lance le chrono de 3s ici
                            prediction_history.clear() 
